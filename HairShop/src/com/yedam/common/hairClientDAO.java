...
public class hairClientDAO extends DAO {
	...
	public int insert1(HairShop client) {

		String sql = "insert into hair_client" + "(client_id,client_name,phone,birth,pay)" + "values(?,?,?,?,?)";
		getConnect();
		try {
			psmt = conn.prepareStatement(sql);
	...
	public List<HairShop> select1() {
		String sql = "select * from hair_client";
	...
	public int update1(HairShop client) {
		String sql = "update hair_client" + " set client_name=?, phone=?, birth=?, pay=?" + " where client_id=?";
		getConnect();
		try {
			psmt = conn.prepareStatement(sql);
			...
			psmt.setInt(5, client.getClientId());
			int r = psmt.executeUpdate();
			return r;
		} catch (SQLException e) {
			e.printStackTrace();
...
//		String today=dateFormat.format(now);//오늘날짜
	...
	public int insert2(HairShop client) {
		...
		String sql = "insert into reservation"
				+ "(reservation,client_name,phone,product,designer)"//
				+ "values(?,?,?,?,?)";
		getConnect();
		try {
			psmt = conn.prepareStatement(sql);
